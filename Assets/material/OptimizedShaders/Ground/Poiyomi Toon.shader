Shader "Hidden/Locked/.poiyomi/Old Versions/9.2/Poiyomi Toon/b3df678a7727fc141a21d8a23c01b3b8"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex, poiUV(poiMesh.uv[0.0].xy, float4(1,1,0,0)), float4(0,0,0,0), 0.0), 4.0);
				#else
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.2300966,0.2300966,0.2300966,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.2300966,0.2300966,0.2300966,1).a;
				#else
