Shader "Hidden/Locked/.poiyomi/Old Versions/9.2/Poiyomi Toon/32133281945a76543bed3e78f5c94746"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 9.2.80</color>", Float) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			VertexOut vert(appdata v)
			{
				...
				float4 outlineMaskTex = _OutlineMask.SampleLevel(sampler_linear_repeat, poiUV(vertexUV(v, 0.0), float4(1,1,0,0)) + _Time.x * float4(0,0,0,0), 0);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0.007581969,0.3837283,0,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0.007581969,0.3837283,0,1).a;
				#else
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods, in PoiCam poiCam)
			{
				clip(1.0 - 0.01);
				#if defined(PROP_OUTLINEMASK) || !defined(OPTIMIZER_ENABLED)
				float OutlineMask = _OutlineMask.SampleLevel(sampler_linear_repeat, TRANSFORM_TEX(poiMesh.uv[0.0], _OutlineMask) + _Time.x * float4(0,0,0,0), 0)[0.0];
				#else
				float OutlineMask = 1;
				#endif
				if (0.0 > 0)
